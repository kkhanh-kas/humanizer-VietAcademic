...
- `SKILL.md` is the repo's only skill file and the prompt agents read. It contains portable YAML metadata, 41 numbered patterns, their examples, and pointers to the bundled files below.
...
- **Patterns:** The skill has 41 numbered patterns. If you add, remove, or renumber a pattern, update the README table, heading, `PATTERN_COUNT` in `scripts/validate-package.py`, and every pattern reference.
- **Personal voice stays out:** This skill holds faults that any Vietnamese academic writer can have. One writer's word choices belong in their own voice layer, described in the README, not in this repo. Keep each pattern in the same section in both files, because the validator compares the groupings.
...
- **The linter has no machine-readable output.** `scripts/kiem_tra.py` prints a
  report for a reader only. A `--format json` flag would let a tool outside this
  repo read its findings. Nothing here needs one yet, because
  `scripts/run-benchmark.py` imports `kiem_tra` and calls `quet()` directly rather
  than parsing the printed text. Build the flag when a caller asks for it.
